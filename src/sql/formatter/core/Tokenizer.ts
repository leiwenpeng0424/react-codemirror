...
export default class Tokenizer {
  ...
  constructor(cfg: TokenizerConfig) {
    ...
    this.NUMBER_REGEX =
      /^((-\s*)?[0-9]+(\.[0-9]+)?|0x[0-9a-fA-F]+|0b[01]+)\b/u
    this.OPERATOR_REGEX =
      /^(!=|<>|==|<=|>=|!<|!>|\|\||::|->>|->|~~\*|~~|!~~\*|!~~|~\*|!~\*|!~|:=|.)/u
    ...
    this.RESERVED_TOP_LEVEL_REGEX = this.createReservedWordRegex(
      cfg.reservedTopLevelWords
    )
    this.RESERVED_TOP_LEVEL_NO_INDENT_REGEX =
      this.createReservedWordRegex(cfg.reservedTopLevelWordsNoIndent)
  ...
  createLineCommentRegex(
    lineCommentTypes: TokenizerConfig["lineCommentTypes"]
  ): RegExp {
    return new RegExp(
      `^((?:${lineCommentTypes
  ...
  createReservedWordRegex(
    reservedWords: TokenizerConfig["reservedWords"]
  ): RegExp {
  ...
  createWordRegex(
    specialChars: TokenizerConfig["specialWordChars"] = []
  ): RegExp {
    return new RegExp(
  ...
  createParenRegex(
    parens:
      | TokenizerConfig["closeParens"]
      | TokenizerConfig["openParens"]
  ): RegExp {
    return new RegExp(
  ...
  createPlaceholderRegex(
    types: TokenizerConfig["indexedPlaceholderTypes"],
    pattern: string
  ): RegExp {
    if (isEmpty(types)) {
      return new RegExp(".*")
  ...
  getPlaceholderTokenWithKey({
    ...
    const token: Token = this.getTokenOnFirstMatch({
      input,
      regex,
  ...
  getReservedWordToken(input: string, previousToken: Token): Token {
